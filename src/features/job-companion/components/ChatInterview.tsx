...
export default function ChatInterview() {
  const { messages, isLoading, initializeSession, sendMessage } = useChatStore();
      ...
      <div className="w-full max-w-3xl h-[70svh] flex flex-col bg-white rounded-xl shadow-2xl">
        ...
        <header className="flex justify-between items-center p-4 bg-accent text-white rounded-t-xl shadow-md">
          <div>
            ...
            <p className="text-xs text-blue-200">Berlatih wawancara dengan AI Interviewer</p>
          </div>
          <Link href={`/job-companion/${'id'}/email-preview`} className="px-4 py-2 text-sm bg-[#0FDB00] font-medium rounded-lg hover:bg-green-500 cursor-pointer transition-colors">Lewati</Link>
        </header>
        ...
        <footer className="p-4 border-t border-slate-200">
          ...
          <p className="text-xs text-slate-500 mt-2 text-center">Tip: Jawab pertanyaan dengan jujur dan jelas seperti dalam wawancara nyata</p>
        </footer>
      </div>
  );
}